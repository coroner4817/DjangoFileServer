...
def file_upload_view(request):
    ...
    if isinstance(form_data.get('file_field'), UploadedFile):
        # If a file is posted, the dummy client should only post the file name,
        # not the full path.
        if os.path.dirname(form_data['file_field'].name) != '':
            return HttpResponseServerError()
        
        document = FileModel.objects.create(testfile=form_data.get('file_field'))
        document.save()
...
def file_upload_view_verify(request):
    ...
    for key, value in form_data.items():
        ...
        if new_hash != submitted_hash.read().decode("utf-8") :
            return HttpResponseServerError()

        if key.startswith('timestamp'):
            timestamp = value.read().decode("utf-8")
            continue;
        document = FileModel.objects.create(testfile=form_data.get(key))
        document.save()
